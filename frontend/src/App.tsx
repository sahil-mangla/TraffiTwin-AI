...
import { EventLogDrawer } from './components/EventTimeline';
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden bg-[#0B0F14] text-[#E8EDF4]">
      ...
      <div className="flex-1 flex min-h-0">
        ...
        {/* Left: Digital Twin canvas — full height, event log overlaid at bottom */}
        <motion.div
          custom={1}
          variants={panelVariants}
          initial="hidden"
          animate="visible"
          className="flex-1 relative min-w-0 min-h-0 p-3"
        >
          <div className="relative w-full h-full overflow-hidden rounded-xl border border-[#2A3545] bg-[#121820] shadow-[0_10px_30px_rgba(0,0,0,0.25)]">
            ...
            {/* Event Log — DustShield-style collapsible overlay */}
            <EventLogDrawer />
          </div>
        </motion.div>

        {/* Right: Operations Rail — slimmed down */}
        <motion.div
          custom={2}
          variants={panelVariants}
          initial="hidden"
      ...
      {/* Control Dock */}
      <motion.div custom={3} variants={panelVariants} initial="hidden" animate="visible">
